...
In the following analysis, we give results for diagnostics preformed on one model, the initial model for overall AQI. The data for this model does not include records with missing or spurious values of the response or predictors. 
...
```{r Large-Leverages, fig.width=5, fig.height=3, echo=FALSE}
#Obtaining leverages
hatv <- hatvalues(lmod)
...
```{r Final NO2 Model}
lambda <- 0.5
lmod.T.NO2 <- lm(formula = AQI ^ lambda ~ Other.GHG + HFC +  Biogenic.CO2 + Population + PFC + HFE + Stationary.Combustion + pp_consumed_MMBtu + poly(Temperature,2) + SF6 + Income, data = na.omit(data_mod_NO2))
