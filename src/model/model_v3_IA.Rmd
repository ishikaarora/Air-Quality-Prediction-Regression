...
plot(data_train$Defining.Parameter,residuals(lmod), xlab="Defining Parameter",ylab="Residuals")
```
...
data_train$AQI <- data_train$AQI + 1
...
+ Temperature + Methane + SF6 + Short.Lived.Compounds + Income + pp_net_gen_MWh, data = data_train)
```
...
step.mod = regsubsets(AQI ~., data = data_train, nvmax = 20, really.big = T, method='forward')
