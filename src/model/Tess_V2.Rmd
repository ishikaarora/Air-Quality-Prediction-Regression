...
```{r}
data_raw <- as.data.frame(read.table("data_merged_meso_weather-dropped.csv", header = T, sep=",", quote=""))
...
```{r}
nogeo.lmod <- lm(formula = AQI ~ Nitrous.Oxide + NF3 + Other.GHG + Total.Emissions + HFC + Other.Fluorane + Biogenic.CO2 + Population + CO2 + PFC + HFE + Stationary.Combustion + pp_consumed_MMBtu + Temperature +  Methane + SF6 + Short.Lived.Compounds + Income +  pp_net_gen_MWh, data = data)
...
```{r}
ozone_nogeo.lmod <- lm(formula = AQI ~ Nitrous.Oxide + NF3 + Other.GHG + Total.Emissions + HFC + Other.Fluorane + Biogenic.CO2 + Population + CO2 + PFC + HFE + Stationary.Combustion + pp_consumed_MMBtu + Temperature +  Methane + SF6 + Short.Lived.Compounds + Income +  pp_net_gen_MWh, data = data_ozone)

summary(ozone_nogeo.lmod)
```
```{r}
ozone_nogeo_2.lmod <- lm(formula = AQI ~ + Biogenic.CO2 + Population +  Stationary.Combustion + pp_consumed_MMBtu + Temperature  + Income +  pp_net_gen_MWh, data = data_ozone)
summary(ozone_nogeo_2.lmod)
```

```{r}
ozone_geo.lmod <- lm(formula = AQI ~ +County+ Biogenic.CO2 + Population +  Stationary.Combustion + pp_consumed_MMBtu + Temperature  + Income +  pp_net_gen_MWh, data = data_ozone)
summary(ozone_geo.lmod)
```

```{r}
ozone_geo_2.lmod <- lm(formula = AQI ~ County + Nitrous.Oxide + NF3 + Other.GHG + Total.Emissions + HFC + Other.Fluorane + Biogenic.CO2 + Population + CO2 + PFC + HFE + Stationary.Combustion + pp_consumed_MMBtu + Temperature +  Methane + SF6 + Short.Lived.Compounds + Income +  pp_net_gen_MWh, data = data_ozone)

summary(ozone_geo_2.lmod)
```
